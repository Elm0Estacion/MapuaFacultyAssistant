...
`;
    ...
    try {
      ...
      // Provide clear actionable guidance if API key is missing
      if (!process.env.GEMINI_API_KEY) {
        return res.status(500).json({
          error: "GEMINI_API_KEY is not set in your .env file. Please add GEMINI_API_KEY=your_key to .env and restart the server.",
        });
      }
    ...
    res.json({ text: responseText || "No response generated by AI model." });
  } catch (error: any) {
